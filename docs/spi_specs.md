...
The conceptual diagram of an SPI bus with two devices is shown in Figure **ADD
FIGURE**. Both the master and the slave have a shift register inside. The two
...
which are the *parallel configuration* and the *daisy-chain configuration*. A
three-slave example is shown in Figure **ADD FIGURE**.

The parallel configuration uses a dedicated **CS** line for each slave device,
as shown in Figure **ADD FIGURE (A)**. An **CS** line functions as the chip
...
A representative timing diagram is shown in Figure **ADD FIGURE**. Initially the
...
Note that the timing diagram in Figure **ADD FIGURE** corresponds to mode 0
since **SCLK** is 0 when it is idle and the first bit is not driven by the first
transition edge.

The timing diagram of four modes is shown in Figure **ADD FIGURE**. Mode 0 is
...
A transaction is shown in Figure **ADD FIGURE**. The edges caused by asserting
...
are eight bits transferred in Figure **ADD FIGURE**. However, the SPI standard does not
...
The `spi_ip` module implements a configurable Serial Peripheral Interface (SPI) master. It supports selectable clock polarity (`cpol_i`) and clock phase (`cpha_i`) modes, programmable clock division (`dvsr_i`), and 8-bit data transfers. The core handles the SPI transaction flow: loading data to transmit, shifting out on `mosi_o`, sampling data from `miso_i`, and signaling completion through `spi_done_tick_o`. A ready flag allows external logic to coordinate new transactions.
...
| `sclk_o`          | Output        | 1         | SPI serial clock output, derived from `clk_i` and controlled by `dvsr_i`, CPOL, CPHA. |
